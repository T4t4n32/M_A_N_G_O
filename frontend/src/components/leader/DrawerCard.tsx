...
interface DrawerCardProps {
  ...
  /** Milestone ids nested inside this card's drawer (empty for Formación, which links out instead). */
  milestoneIds?: string[];
}

/**
 * "Foto de fondo" (a colored panel with a big centered icon — these four
 * items don't have real uploaded photos, they're conceptual/profile cards)
 * + label. Click opens a Drawer with the full description and — where the
 * old inline-expanding chips had real dated milestones nested underneath
 * (Inicios → Inicios+RobiSoft, Visión → ECOLATAS, Reconocimientos → the two
 * school recognitions) — those same milestone previews, each still opening
 * the full ImmersivePanel on its own click. Formación has no milestones to
 * nest; its CTA is the real external YouTube channel link instead.
 */
export function DrawerCard({ card, milestoneIds = [] }: DrawerCardProps) {
  ...
  const trigger = (
    <button
      type="button"
      onClick={card.externalHref ? undefined : () => setOpen(true)}
    ...
    >
      ...
      >
        {card.externalHref && <Youtube className="h-3 w-3" />}
        {card.ctaLabel}
        {card.externalHref && <ExternalLink className="h-2.5 w-2.5 opacity-60" />}
      </span>
    </button>
  );
  ...
  return (
    <>
      {card.externalHref ? (
        <a href={card.externalHref} target="_blank" rel="noopener noreferrer" className="block">
          {trigger}
        </a>
      ) : (
        trigger
