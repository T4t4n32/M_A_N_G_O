...
function createTextTexture(gl: GL, text: string, font = 'bold 30px monospace', color = 'black', maxWidth = 260) {
...
class App {
  ...
  constructor(container: HTMLElement, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onItemClick }: AppConfig = {}) {
  ...
  onTouchUp(e: MouseEvent | TouchEvent) {
    if (this.isDown) {
      ...
      if (dx < 5 && dy < 5 && this.onItemClick && this.medias) {
        // Find closest media to center
        ...
        if (closest) {
          ...
          const originalItems = this.mediasImages.slice(0, this.mediasImages.length / 2);
          if (originalItems[idx]) {
            this.onItemClick(originalItems[idx]);
          }
        }
      }
    }
