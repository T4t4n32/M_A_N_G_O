...
export function DocumentationSection() {
  ...
  const filtered = useMemo(() => {
    let result = active === "Todos" ? docs : docs.filter((d) => d.category === active);
    if (activeFormat) {
      result = result.filter((d) => d.files.some((f) => f.label === activeFormat));
    }
    if (search.trim()) {
      ...
      result = result.filter(
        ...
        d.files.some((f) => f.label.toLowerCase().includes(q))
      );
    }
    return result;
  }, [active, activeFormat, search]);

  const canCollapse = filtered.length > INITIAL_VISIBLE;
  const visible = expanded || !canCollapse ? filtered : filtered.slice(0, INITIAL_VISIBLE);

  const counts = categoryFilters.map((f) => ({
    label: f,
    count: f === "Todos" ? docs.length : docs.filter((d) => d.category === f).length
  }));

  const getFormatCount = (fmt: string) => docs.filter((d) => d.files.some((f) => f.label === fmt)).length;

  return (
    <section id="documentacion" className="py-20 md:py-28 bg-mango-dark relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_60%,hsl(168_72%_42%/0.04),transparent_50%)]" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-bold text-white">Documentación</h2>
          <p className="mt-3 text-white/50 max-w-2xl mx-auto">
            Recursos, informes y documentos del proyecto — {docs.length} archivos disponibles
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-white/50">
            {[
            { label: "Presentación", desc: "Última presentación oficial", ext: ".pptx", icon: Presentation, href: "/docs/LATEST_PRESENTATION_MANGO.pptx", style: formatIcons.PPTX, gradient: "from-orange-500/20 to-orange-600/5" },
            { label: "Informe", desc: "Documento principal del proyecto", ext: ".docx", icon: FileText, href: "/docs/M_A_N_G_O.docx", style: formatIcons.DOCX, gradient: "from-blue-500/20 to-blue-600/5" },
            ...
            { label: "Bitácora", desc: "Última bitácora de etapa productiva", ext: ".xlsx", icon: FileSpreadsheet, href: "/docs/BITACORA_6_SEBASTIAN_SANCHEZ_CHACON.xlsx", style: formatIcons.XLSX, gradient: "from-emerald-500/20 to-emerald-600/5" }].
            map(({ label, desc, ext, icon: FmtIcon, href, style, gradient }) =>
            <a
              key={label}
              href={href}
              download
              className={`group/card relative flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-300 bg-gradient-to-b ${gradient} ${style.border} hover:shadow-xl hover:shadow-current/5 hover:scale-[1.05] hover:border-opacity-60 cursor-pointer overflow-hidden`}>
          ...
          <div className="mt-4 flex justify-center">
            <a
              href="/docs/Informe_Tecnico_MANGO.md"
              download
              className={`group flex items-center gap-2.5 px-5 py-2.5 rounded-xl border-2 transition-all duration-300 text-sm bg-gradient-to-r from-purple-500/15 to-purple-600/5 ${formatIcons.MD.border} ${formatIcons.MD.color} hover:shadow-lg hover:scale-[1.03]`}>
              
              <FileType className="h-4 w-4" />
              <span className="font-semibold">Informe Técnico</span>
              <Download className="h-3.5 w-3.5 opacity-60 group-hover:animate-bounce" />
        ...
        <div className="max-w-4xl mx-auto mb-4 bg-white/[0.04] backdrop-blur-sm rounded-xl border border-white/[0.08] p-4">
          <Collapsible>
            <CollapsibleTrigger className="w-full flex items-center justify-between text-sm font-semibold text-white/70 hover:text-white transition-colors cursor-pointer">
              <span className="flex items-center gap-2">
                <span className="p-1 rounded-md bg-[hsl(195,70%,48%)]/10">
                  <Search className="h-3.5 w-3.5 text-[hsl(195,70%,48%)]" />
                </span>
                Filtrar por tipo de archivo
              </span>
              <span className="flex items-center gap-2">
                {activeFormat &&
                <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${formatIcons[activeFormat]?.bg} ${formatIcons[activeFormat]?.color} ${formatIcons[activeFormat]?.border} border`}>
            ...
            <CollapsibleContent>
              <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-white/[0.08]">
                <button
                  onClick={() => {setActiveFormat(null);setExpanded(false);}}
                ...
                {Object.keys(formatIcons).map((fmt) => {
                  const style = formatIcons[fmt];
                  const isActive = activeFormat === fmt;
                  return (
                    <button
                      key={fmt}
                      onClick={() => {setActiveFormat(isActive ? null : fmt);setExpanded(false);}}
                      className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 border-2 ${
                      isActive ?
                      ...
                      .{fmt.toLowerCase()} <span className="opacity-50">({getFormatCount(fmt)})</span>
                    </button>);
          ...
          <Collapsible open={catOpen} onOpenChange={setCatOpen}>
            ...
            <CollapsibleContent>
              <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-white/[0.08]">
                ...
                <button
                  key={label}
                  onClick={() => {setActive(label);setExpanded(false);}}
                  className={`px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 border-2 ${
                  active === label ?
                  "bg-[hsl(195,70%,48%)]/20 text-[hsl(195,70%,70%)] border-[hsl(195,70%,48%)]/40 shadow-md shadow-[hsl(195,70%,48%)]/10" :
                  "bg-white/[0.04] text-white/50 border-white/[0.1] hover:border-white/[0.2] hover:text-white/70"}`
        ...
        <div className="max-w-4xl mx-auto mb-6 flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[hsl(195,70%,48%)]/15 text-[hsl(195,70%,70%)] text-xs font-medium border border-[hsl(195,70%,48%)]/20">
            ...
            {activeFormat &&
          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${formatIcons[activeFormat].bg} ${formatIcons[activeFormat].color} ${formatIcons[activeFormat].border}`}>
                .{activeFormat.toLowerCase()}
                <button onClick={() => setActiveFormat(null)} className="hover:opacity-70"><X className="h-3 w-3" /></button>
              </span>
          }
            <button
            onClick={() => {setActive("Todos");setActiveFormat(null);setSearch("");}}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            return (
                <div
                key={`${doc.title}-${i}`}
                className="bg-white/[0.04] backdrop-blur-sm rounded-xl border border-white/[0.08] p-5 hover:-translate-y-1 hover:shadow-lg hover:shadow-[hsl(195,70%,48%)]/5 transition-all duration-300 group">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.08]">
                        ...
                        <span
                          key={j}
                          className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${formatColors[file.label] || "bg-muted text-muted-foreground"}`}>
                        ...
                        <Button
                          ...
                          size="sm"
                          variant="ghost"
                          className="text-[hsl(195,70%,60%)] hover:text-[hsl(195,70%,80%)] hover:bg-white/[0.06] gap-1 text-xs h-7 px-2"
                          asChild>
        ...
        <div className="flex justify-center mt-8">
            ...
            className="gap-2 rounded-full px-6 border-white/[0.15] text-white/60 hover:text-white hover:bg-white/[0.08] bg-white/[0.04]">
            ...
            <>
                  Ver más ({filtered.length - INITIAL_VISIBLE} restantes) <ChevronDown className="h-4 w-4" />
                </>
            }
            </Button>
          </div>
