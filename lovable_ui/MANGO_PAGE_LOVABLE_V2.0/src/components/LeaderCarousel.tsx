...
import { ChevronLeft, ChevronRight, Play, X, Camera, ZoomIn, ZoomOut, RotateCcw } from "lucide-react";
      ...
      <div className="mt-6">
        ...
        <div className="relative rounded-xl overflow-hidden bg-white/[0.04] border border-white/[0.08]">
          <div
            ...
            onClick={() => { setLightbox(current); setZoom(1); }}
          >
            {item.type === "image" ? (
              <img src={item.src} alt={item.alt} className="w-full h-full object-cover" loading="lazy" />
            ) : (
              <video src={item.src} className="w-full h-full object-cover" muted playsInline loop autoPlay />
            )}
          </div>
          ...
          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent px-3 py-2.5">
            <div className="flex items-center justify-between">
              ...
              <span className="text-white/50 text-xs font-mono shrink-0">{current + 1} / {filtered.length}</span>
            </div>
            <div className="flex gap-1 mt-1.5 justify-center">
              {filtered.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${i === current ? "w-4 bg-accent" : "w-1.5 bg-white/30"}`}
                />
              ))}
            </div>
          </div>
        ...
        >
          ...
          {filtered[lightbox]?.type === "image" && (
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 flex items-center gap-1 bg-white/10 backdrop-blur-sm rounded-full px-2 py-1">
              <button onClick={(e) => { e.stopPropagation(); setZoom(Math.max(0.5, zoom - 0.25)); }} className="p-1.5 text-white/70 hover:text-white">
                <ZoomOut className="h-4 w-4" />
              </button>
              <span className="text-white/80 text-xs font-medium min-w-[3rem] text-center">{Math.round(zoom * 100)}%</span>
              <button onClick={(e) => { e.stopPropagation(); setZoom(Math.min(3, zoom + 0.25)); }} className="p-1.5 text-white/70 hover:text-white">
                <ZoomIn className="h-4 w-4" />
              </button>
              {zoom !== 1 && (
                <button onClick={(e) => { e.stopPropagation(); setZoom(1); }} className="p-1.5 text-white/70 hover:text-white">
          ...
          <div className="max-w-5xl max-h-[90vh] w-full px-4 sm:px-14 md:px-20" onClick={(e) => e.stopPropagation()}>
            {filtered[lightbox]?.type === "image" ? (
            ...
            <p className="text-white/60 text-xs text-center mt-3">{filtered[lightbox]?.alt}</p>
          </div>
